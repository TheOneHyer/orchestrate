...
/**
 * Merges `updates` into `entity` when `entity.id === id`, stamping a fresh
 * `updatedAt` timestamp. If the caller supplies an `updatedAt` in `updates`
 * that differs from the one already stored on `entity`, a concurrent-edit
 * warning toast is shown before the merge is applied (last-write-wins).
 * Entities whose `id` does not match are returned unchanged.
 *
 * @param entity - The existing record to update.
 * @param id - Target entity ID; non-matching entities are returned as-is.
 * @param updates - Partial fields to merge in (including an optional `updatedAt` staleness sentinel).
 * @param warningMessage - Human-readable description shown in the concurrent-edit toast.
 * @returns The updated entity with a refreshed `updatedAt` timestamp.
 */
/**
 * Constructs a complete {@link Session} from partial input, filling in safe
 * defaults for every omitted field and stamping `updatedAt` with `now`.
 *
 * @param session - Caller-supplied partial session data.
 * @param now - ISO timestamp applied to `updatedAt` (and used as the default `startTime`/`endTime` when absent).
 * @returns A fully-populated session object ready for storage.
 */
function buildSessionDefaults(session: Partial<Session>, now: string): Session {
  return {
...
function applyEntityUpdates<T extends { id: string; updatedAt?: string }>(
  entity: T,
  id: string,
  updates: Partial<T>,
  ...
  const { updatedAt: expectedUpdatedAt, ...restUpdates } = updates as T & { updatedAt?: string }

  if (expectedUpdatedAt && entity.updatedAt && expectedUpdatedAt !== entity.updatedAt) {
    toast.error('Concurrent edit warning', { description: warningMessage })
  }

  return { ...entity, ...(restUpdates as Partial<T>), updatedAt: new Date().toISOString() }
}
...
function App() {
  ...
  const applyPreviewSeedData = useCallback((seedMode: PreviewSeedMode | 'manual' = 'manual') => {
    ...
    setAuthPasswords(Object.fromEntries(seedData.users.map((user) => [user.id, 'password123'])))
    ...
    if (!previewMode && useServerAuth) {
      try {
        ...
        const authenticatedUser = authenticatedUserId
          ? safeUsers.find((user) => user.id === authenticatedUserId)
          : undefined
        ...
        setNavigationPayload(null)
        signInForm.setValue('password', '')
        toast.success('Signed in', { description: `Welcome back, ${authenticatedUser.name}.` })
      } catch {
        toast.error('Authentication failed', {
  ...
  const handleCreateSession = (session: Partial<Session>) => {
    const newSession = buildSessionDefaults(session, new Date().toISOString())
  ...
  const handleCreateMultipleSessions = (sessions: Partial<Session>[]) => {
    const now = new Date().toISOString()
    const newSessions = sessions.map((session) => buildSessionDefaults(session, now))
  ...
  const handleUpdateSession = (id: string, updates: Partial<Session>) => {
    setSessions((currentSessions) =>
      (currentSessions || []).map((session) => applyEntityUpdates(session, id, updates, 'This session changed since you opened it. Your latest update was saved with last-write-wins.'))
    )
  }
  ...
  const handleUpdateUser = useCallback((id: string, updates: Partial<User>) => {
    setUsers((currentUsers) =>
      (currentUsers || []).map((user) => applyEntityUpdates(user, id, updates, 'This profile changed since you opened it. Your latest update was saved with last-write-wins.'))
    )
  }, [setUsers])
  ...
  const handleUpdateCourse = useCallback((id: string, updates: Partial<Course>) => {
    setCourses((currentCourses) =>
      (currentCourses || []).map((course) => applyEntityUpdates(course, id, updates, 'This course changed since you opened it. Your latest update was saved with last-write-wins.'))
    )
  }, [setCourses])
