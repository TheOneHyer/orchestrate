...
/**
 * Render the Courses library view with search, filtering, per-course progress, and (role-gated) creation UI.
 *
 * Displays a searchable grid of course cards showing title, description, duration, module count,
 * certifications, and the current user's enrollment progress. When the current user is an admin
 * or trainer, a Create Course workflow is available.
 *
 * @param courses - Array of courses to display.
 * @param enrollments - Enrollment records used to render per-course progress for the current user.
 * @param currentUser - The authenticated user; role controls visibility of course creation actions.
 * @param onNavigate - Navigation callback invoked for in-app navigation actions (card open, create).
 * @param onCreateCourse - Optional callback to persist a newly created course; may be synchronous or return a promise.
 * @param navigationPayload - Optional one-time payload that can open the create dialog or a course detail view.
 * @param onNavigationPayloadConsumed - Optional callback invoked after a provided navigationPayload has been processed.
 * @returns The React element representing the Courses page.
 */
