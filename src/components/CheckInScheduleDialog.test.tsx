...
describe('CheckInScheduleDialog', () => {
  ...
  it('shows a validation error when custom days exceed the maximum', async () => {
    ...
    await user.clear(screen.getByLabelText(/custom days/i))
    await user.type(screen.getByLabelText(/custom days/i), '366')
    await user.click(screen.getByRole('button', { name: /create schedule/i }))

    expect(onSubmit).not.toHaveBeenCalled()
    expect(screen.getByText(/custom days must be 365 or fewer/i)).toBeInTheDocument()
  })

  it('shows reminder validation when an existing schedule has negative reminder hours', async () => {
    const user = userEvent.setup()
    const onSubmit = vi.fn()
    const existing = makeExistingSchedule({ reminderHoursBefore: -1 })
    ...
    expect(onSubmit).not.toHaveBeenCalled()
    expect(screen.getByText(/reminder hours cannot be negative/i)).toBeInTheDocument()
  })
